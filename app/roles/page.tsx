...
type Role = { id: number; name: string; active: boolean; comment: string };
...
export default function RolesPage() {
    ...
    function handleEdit(role: Role) {
        setEditForm({id: role.id, name: role.name, comment: role.comment ?? '', active: role.active});
    ...
    async function handleSaveEdit() {
        ...
        setEditError(null);
        try {
            const res = await fetch(`/api/roles/${editForm.id}`, {
                method: 'PATCH',
                headers: {'Content-Type': 'application/json'},
            ...
            setRoles(prev => prev.map(r => r.id === editForm.id ? data.role : r));
            setEditModalOpen(false);
        } catch {
    ...
    return (
        <div className="flex min-h-screen bg-gray-100">
            ...
            <Modal isOpen={modalOpen} onClose={handleCloseModal} title="Добавление роли">
                <div className="flex flex-col gap-4">
                    <Input label="Название роли" name="name" value={form.name} onChange={handleChange}/>
            ...
            <Modal isOpen={editModalOpen} onClose={handleCloseEditModal} title="Изменение роли">
                <div className="flex flex-col gap-4">
            ...
            <Modal isOpen={deleteId !== null} onClose={() => setDeleteId(null)} title="Подтверждение удаления">
                <p className="text-sm text-gray-600 mb-6">
                    Вы уверены, что хотите удалить роль <span className="font-medium text-gray-800">
