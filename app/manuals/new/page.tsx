...
export default function NewManualPage() {
    ...
    useEffect(() => {
        ...
        fetch('/api/manuals').then(r => r.json())
            .then(d => setManualOptions(
                (d.manuals ?? [])
                    .filter((m: { id: number }) => String(m.id) !== manualId)
                    .map((m: { id: number; title: string }) => ({ value: String(m.id), label: m.title }))
            )).catch(() => {});
        ...
        if (isEditMode) {
            ...
            fetch(`/api/manuals/${manualId}`)
                ...
                .then(data => {
                    ...
                    if (m.type !== 'text') setCurrentContentUrl(m.content ?? null);
                    else setContent(m.content ?? '');
                })
                .catch(() => {})
    ...
    async function handleSave() {
        ...
        try {
            ...
            const url    = isEditMode ? `/api/manuals/${manualId}` : '/api/manuals';
    ...
    const iconDisplay = iconPreview ?? currentIcon;
                ...
                <main className="flex-1 p-6 max-w-3xl">
                    <div className="flex items-center justify-between mb-6">
                        <h3 className="text-xl font-semibold text-gray-800">
                            {isEditMode ? 'Редактировать материал' : 'Новый материал'}
                        </h3>
                    </div>

                    {loading ? (
                        <div className="bg-white rounded-2xl shadow-sm p-6 text-center text-gray-400">Загрузка...</div>
                    ) : (
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <div className="flex items-end gap-3">
                                            ...
                                            <Input label={isEditMode ? 'Сменить иконку' : 'Иконка'} type="fileUpload" accept="image/*" onChange={handleIconChange} />
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="flex flex-col gap-3">
                                            ...
                                            {contentPreview && form.type === 'video' && (
                                                <video src={contentPreview} controls className="w-full rounded-lg max-h-52 bg-black" />
                                            )}
